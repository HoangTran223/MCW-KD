...
def finetune(
    ...
    """
    Main training loop for MCW-KD with ε-Entropic Dual Form.
    """
    log_rank("=" * 60)
    log_rank("Starting MCW-KD Training with ε-Entropic Dual Form")
    log_rank("=" * 60)
    log_rank("Start measuring training time")
    ...
    # Adaptive weight update settings
    update_interval = getattr(args, 'weight_update_interval', 1000)
    step_since_last_update = 0
    cost_values_logits_buffer = []
    cost_values_hidden_buffer = []

    log_rank(f"Adaptive weight update interval: {update_interval} steps")
    ...
    for epoch in range(args.num_epochs):
        ...
        epoch_ot_logits, epoch_ot_hidden = 0.0, 0.0
        ...
        while True:
            ...
            for batch in global_batch:
                ...
                # NOTE: φ is updated in inner loop

                torch.cuda.synchronize()
                elapsed_time = time.time() - st_time
                logging_output["micro_step_time"].append(elapsed_time)

                # Collect cost values for adaptive weight update
                logits_cost_keys = ["avg_c1_logits", "avg_c2_logits", "avg_c3_logits"]
                hidden_cost_keys = ["avg_c1_hidden", "avg_c2_hidden", "avg_c3_hidden"]
                
                alt_logits_keys = ["avg_c2_logits", "avg_c4_logits", "avg_c_salience_logits"]
                alt_hidden_keys = ["avg_c2_last", "avg_c5_last", "avg_c6_last"]
                ...
                try:
                    # Try new keys first
                    if all(k in batch_logging_output for k in logits_cost_keys):
                        cost_values_logits = [to_scalar(batch_logging_output[k]) for k in logits_cost_keys]
                    elif all(k in batch_logging_output for k in alt_logits_keys):
                        cost_values_logits = [to_scalar(batch_logging_output[k]) for k in alt_logits_keys]
                    else:
                        cost_values_logits = None

                    if all(k in batch_logging_output for k in hidden_cost_keys):
                        cost_values_hidden = [to_scalar(batch_logging_output[k]) for k in hidden_cost_keys]
                    elif all(k in batch_logging_output for k in alt_hidden_keys):
                        cost_values_hidden = [to_scalar(batch_logging_output[k]) for k in alt_hidden_keys]
                    else:
                        cost_values_hidden = None

                    if cost_values_logits is not None and cost_values_hidden is not None:
                        cost_values_logits_buffer.append(cost_values_logits)
                        cost_values_hidden_buffer.append(cost_values_hidden)
                except Exception as e:
                    if step % 100 == 0:
                        print(f"Warning: Failed to collect cost values: {e}")
            ...
            if hasattr(criterion, "update_cost_weights") and step_since_last_update >= update_interval:
                if cost_values_logits_buffer and cost_values_hidden_buffer:
                    expected_size = update_interval * args.gradient_accumulation_steps
                    
                    log_rank(f"[Step {global_step}] Updating cost weights...")
                    log_rank(f"  Buffer size: {len(cost_values_logits_buffer)} (expected ~{expected_size})")
                    
                    # Average cost values
                    avg_cost_logits = torch.tensor(cost_values_logits_buffer).mean(dim=0).tolist()
                    avg_cost_hidden = torch.tensor(cost_values_hidden_buffer).mean(dim=0).tolist()
                    
                    log_rank(f"  Avg logits costs: {avg_cost_logits}")
                    log_rank(f"  Avg hidden costs: {avg_cost_hidden}")
                    
                    criterion.update_cost_weights(
                        cost_values_logits=avg_cost_logits,
                        cost_values_hidden=avg_cost_hidden
                    )
            ...
            def get_log(logging_output):
                logging_info = ""
                # Keys that contain lists of lists (not scalars) - skip averaging
                skip_keys = ["alpha_logits", "alpha_hidden", "cost_values_last", "cost_values_first"]
                for key in logging_output:
                    if key == "epoch" or key in skip_keys:
                        continue
                    log_val = logging_output[key]
                    if isinstance(log_val, list) and len(log_val) > 0:
                        # Check if first element is a list (nested list)
                        if isinstance(log_val[0], list):
                            continue  # Skip nested lists
                        try:
                            logging_info += f"{key}={sum(log_val) / len(log_val):.4f}, "
                        except TypeError:
                            continue  # Skip if can't sum
            ...
            if logging_output["global_step"] % args.log_interval == 0:
                ...
                if "ot_loss_logits" in logging_output and logging_output["ot_loss_logits"]:
                    epoch_ot_logits += sum(logging_output["ot_loss_logits"])
                if "ot_loss_hidden" in logging_output and logging_output["ot_loss_hidden"]:
                    epoch_ot_hidden += sum(logging_output["ot_loss_hidden"])
        ...
        log_rank("train | epoch {:0>3d} | loss {:.4f} | nll_loss {:.4f} | kd_loss {:.4f} | ot_logits {:.4f} | ot_hidden {:.4f}".format(
            epoch + 1,
            epoch_loss / max(epoch_step * args.gradient_accumulation_steps, 1),
            epoch_nll_loss / max(epoch_step * args.gradient_accumulation_steps, 1),
            epoch_kd_loss / max(epoch_step * args.gradient_accumulation_steps, 1),
            epoch_ot_logits / max(epoch_step * args.gradient_accumulation_steps, 1),
            epoch_ot_hidden / max(epoch_step * args.gradient_accumulation_steps, 1),
        ))
        ...
        if args.save_dir and (epoch + 1) % args.save_interval == 0:
            if (epoch + 1) % args.eval_interval == 0:
                ...
                if dist.get_rank() == 0:
                    ...
                    # Save criterion state (cost weights)
                    if hasattr(criterion, "cost_weights_logits"):
                        criterion_state = {
                            "cost_weights_logits": criterion.cost_weights_logits.data,
                            "cost_weights_hidden": criterion.cost_weights_hidden.data,
                        }
                        torch.save(criterion_state, os.path.join(save_dir_path, "criterion_state.pt"))
                        log_rank("Saved criterion state (cost weights)")
    ...
    log_rank("Total time: {:0>2}:{:0>2}:{:0>2}".format(
...
def main():
    ...
    log_rank("Initializing distiller for MCW-KD with ε-Entropic Dual Form...")
    ...
    # Build criterion (with learnable φ, β, etc.)
    # φ is updated in inner loop of MultiCostSemiDualOT via manual gradient steps
    # β starts at zeros → α = softmax(β) = uniform weights (acceptable starting point)
    criterion = build_criterion(args)
    ...
    if args.do_train:
        finetune(args, distiller.student_tokenizer, model, optimizer, lr_scheduler, dataset, device, criterion=criterion)
